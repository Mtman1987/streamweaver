...
export default function IntegrationsPage() {
  ...
  const buildAuthHeaders = (json = true): HeadersInit => {
  ...
  useEffect(() => {
    const checkPlatformConfig = async () => {
      ...
      try {
        const statusRes = await fetch('/api/platforms/status', {
        ...
        setPlatformStates(prev => ({
          ...prev,
          youtube: Boolean(status.youtubeConnected),
          kick: Boolean(status.kickConnected),
          tiktok: Boolean(status.tiktokConnected),
          discord: Boolean(status.discordConnected),
        }));
      } catch {
  ...
  const connectTwitch = (role: 'broadcaster' | 'bot' | 'community-bot') => {
    ...
    if (role === 'bot') {
      toast({
        title: '⚠️ Use your BOT account',
        description: 'On the Twitch login page, sign in as your BOT account — not your personal/broadcaster account. They must be different.',
      });
    }
  ...
  const testAndSaveObs = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        // Settings saved even if server test fails - browser bridge handles connection
        toast({
          title: "OBS settings saved",
          description: "The dashboard will connect to OBS from your browser.",
        });
    ...
    <div className="space-y-6">
                ...
                <div className="flex items-center justify-between rounded border px-3 py-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">Your main Twitch account — connected automatically when you sign in</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {twitchStatus.broadcasterConnected ? (
                      <>
                        ...
                        <Button size="sm" variant="outline" onClick={() => connectTwitch('broadcaster')}>
                          Re-authorize
                        </Button>
                        <Button size="sm" variant="destructive" onClick={() => void disconnectTwitch('broadcaster')}>
                          Disconnect
                        </Button>
                      </>
                    ) : (
                ...
                <div className="flex items-center justify-between rounded border px-3 py-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {!twitchStatus.botConnected && (
                        <div className="text-xs text-amber-500 font-medium">Must be a different account than Broadcaster</div>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center justify-between rounded border px-3 py-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-purple-600 font-medium">Shared bot for Chat Tag & cross-stream features</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {twitchStatus.communityBotConnected ? (
                      ...
                      <Button 
                        ...
                        onClick={async () => {
                          if (platform.id === 'discord') {
                            toast({
                              title: 'Discord connection is env-based',
                              description: 'Update DISCORD_BOT_TOKEN to connect or disconnect Discord.',
                            });
                            return;
                          }

                          if (platform.id === 'youtube' || platform.id === 'kick' || platform.id === 'tiktok') {
                            if (!requireLocalApiKey()) return;

                            try {
                              const response = await fetch(`/api/platforms/${platform.id}/connect`, {
                                method: 'DELETE',
                                headers: buildAuthHeaders(false),
                              });
                              ...
                              setPlatformStates({ ...platformStates, [platform.id]: false });
                              toast({ title: `${platform.name} disconnected` });
                            } catch (error: any) {
                              toast({
                                variant: 'destructive',
                                title: `Failed to disconnect ${platform.name}`,
                                description: String(error?.message || error),
                              });
                            }
                            return;
                          ...
                          onClick={() => {
                            ...
                            if (username) {
                              fetch(`/api/platforms/${platform.id}/connect`, {
                                ...
                                toast({ title: `${platform.name} connected`, description: `Connected as ${username}` });
                              }).catch((error: any) => {
                                toast({
                                  variant: 'destructive',
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Connect to OBS WebSocket server for scene control and automation. The dashboard bridges commands to OBS — keep this page open while streaming.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle>OBS Scene Names</CardTitle>
          <CardDescription>
            Tell StreamWeaver what your OBS scenes are called. Commands like !brb will switch to the scene name you enter here.
            These must match your OBS scene names exactly.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {[
            { key: 'live', label: 'Live / Main Scene', hint: 'Your main streaming scene (e.g. "Live", "Main", "Gameplay")' },
            { key: 'brb', label: 'BRB Scene', hint: 'Shown when you step away (e.g. "BRB", "Be Right Back")' },
            { key: 'starting', label: 'Starting Soon Scene', hint: 'Pre-stream waiting screen (e.g. "Starting Soon")' },
            { key: 'ending', label: 'Ending Scene', hint: 'Stream outro scene (e.g. "Ending", "Outro")' },
            { key: 'chatting', label: 'Just Chatting Scene', hint: 'Webcam/chat focused scene (e.g. "Just Chatting", "Cam")' },
            { key: 'gaming', label: 'Gaming Scene', hint: 'Fullscreen game capture (e.g. "Gaming", "Game")' },
          ].map(({ key, label, hint }) => (
            <div key={key} className="space-y-1">
              <Label htmlFor={`obs-scene-${key}`}>{label}</Label>
              <Input
                id={`obs-scene-${key}`}
          ...
          <Button onClick={saveObsScenes} disabled={obsScenesBusy}>
            {obsScenesBusy ? 'Saving...' : 'Save Scene Names'}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Sign out of StreamWeaver and return to the login page.</CardDescription>
        </CardHeader>
        <CardContent>
          <Button
            variant="destructive"
