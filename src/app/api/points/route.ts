...
export async function GET(request: NextRequest) {
  ...
  const fallbackCtx = tenantParam
    ? {
        tenantId: tenantParam,
        username: readUserConfigSync(tenantParam).TWITCH_BROADCASTER_USERNAME || 'default',
      }
    : undefined;
  const ctx = session ? toStorageContext(session) : fallbackCtx;

  try {
    if (action === 'leaderboard') {
      const leaderboard = await getLeaderboard(limit, ctx);
      return apiOk({ leaderboard });
    }

    if (action === 'get' && userId) {
      const userPoints = await getPoints(userId, ctx);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const session = getTenantFromRequest(request);
    const ctx = session ? toStorageContext(session) : undefined;

    // Backwards-compatible: accept { username, points } payloads from UI
    if (!action && username && typeof points === 'number') {
      const result = await addPoints(username, points, reason || 'ui action', ctx);
    ...
    if (action === 'add') {
      ...
      const result = await addPoints(targetUser, amount, 'manual add', ctx);
    ...
    if (action === 'set') {
      ...
      const result = await setPoints(targetUser, value, ctx);
      return apiOk({ userId: targetUser, ...result });
    }
