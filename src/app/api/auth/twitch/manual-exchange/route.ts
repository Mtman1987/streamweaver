...
import { tenantPath, communityBotTokensPath, isAdmin, getTenantIdFromSession } from '@/lib/tenant';
import { z } from 'zod';

const manualExchangeSchema = z.object({
  code: z.string().trim().min(1).max(4096),
  state: z.enum(['broadcaster', 'bot', 'community-bot', 'login']).optional().default('broadcaster'),
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!tenantId && state !== 'login') {
      return apiError('Session required — login first', { status: 401, code: 'NO_SESSION' });
    }
    ...
    if (state === 'community-bot') {
      if (!tenantId || !isAdmin(tenantId)) {
        return apiError('Admin only', { status: 403, code: 'ADMIN_ONLY' });
      }
    ...
    if (!tenantId) {
      return apiError('Session required', { status: 401, code: 'NO_SESSION' });
    }
