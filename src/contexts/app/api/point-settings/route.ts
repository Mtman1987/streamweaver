...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (type === 'rewards') {
      const rewards = await getChannelPointRewards(ctx);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === 'rewards') {
      ...
      await updateChannelPointRewards(rewards, ctx);
      const updated = await getChannelPointRewards(ctx);
      return apiOk({ rewards: updated });
    }
    
    const settings = pointSettingsSchema.parse(data);
    await updatePointSettings(settings as Partial<PointSettings> & { eventPoints?: Partial<PointSettings['eventPoints']> }, ctx);
    const updated = await getPointSettings(ctx);
