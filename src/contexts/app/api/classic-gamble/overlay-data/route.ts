...
export async function GET(req: NextRequest) {
    try {
        ...
        const tenantOverlayPath = tenantId
            ? tenantPath(tenantId, 'data/masterstats/overlay/classic-gamble.json')
            : '';
        const globalOverlayPath = path.resolve(process.cwd(), 'data', 'masterstats', 'overlay', 'classic-gamble.json');
