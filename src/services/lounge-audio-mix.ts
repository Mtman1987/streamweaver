...
export type LoungeMixOutput = 'stella' | 'spotlight' | 'media';
...
const DEFAULTS: LoungeAudioMix = {
  levels: { stella: 100, spotlight: 58, media: 85 },
  selected: 'media',
  updatedAt: 0,
};
const OUTPUTS = new Set<LoungeMixOutput>(['stella', 'spotlight', 'media']);
...
function normalize(input: Partial<LoungeAudioMix> | null): LoungeAudioMix {
  const volume = (output: LoungeMixOutput) => {
    ...
    return Number.isInteger(value) && value >= 1 && value <= 100 ? value : DEFAULTS.levels[output];
  };
  return {
    levels: { stella: volume('stella'), spotlight: volume('spotlight'), media: volume('media') },
...
export async function selectLoungeMixOutput(output: LoungeMixOutput): Promise<LoungeAudioMix> {
  if (!OUTPUTS.has(output)) throw new Error('Choose Stella, Spotlight, or media.');
...
export async function setLoungeMixVolume(output: LoungeMixOutput | null, value: number): Promise<LoungeAudioMix> {
  if (!Number.isInteger(value) || value < 1 || value > 100) throw new Error('Volume must be 1-100.');
  const state = await getLoungeAudioMix();
